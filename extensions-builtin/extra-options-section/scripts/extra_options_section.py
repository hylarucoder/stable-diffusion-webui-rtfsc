...
class ExtraOptionsSection(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Blocks() as interface:
            with gr.Accordion(
                "Options", open=False
            ) if shared.opts.extra_options_accordion and shared.opts.extra_options else gr.Group(), gr.Row():
        ...
        def get_settings_values():
            return [
                ui_settings.get_value_for_setting(key) for key in self.setting_names
            ]
...
shared.options_templates.update(
    shared.options_section(
        ("ui", "User interface"),
        {
            "extra_options": shared.OptionInfo(
                [],
                "Options in main UI",
            ...
            .info("setting entries that also appear in txt2img/img2img interfaces")
            .needs_restart(),
            "extra_options_accordion": shared.OptionInfo(
                False, "Place options in main UI into an accordion"
            ),
        },
    )
)
