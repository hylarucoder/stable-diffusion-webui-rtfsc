...
def create_seed_inputs(target_interface):
    ...
    with FormRow(
        visible=False, elem_id=f"{target_interface}_subseed_row"
    ) as seed_extra_row_1:
        seed_extras.append(seed_extra_row_1)
        subseed = gr.Number(
            label="Variation seed", value=-1, elem_id=f"{target_interface}_subseed"
        )
...
def setup_ui_api(app):
    ...
    def download_sysinfo(attachment=False):
        ...
        text = sysinfo.get()
        filename = (
            f"sysinfo-{datetime.datetime.utcnow().strftime('%Y-%m-%d-%H-%M')}.txt"
        )
