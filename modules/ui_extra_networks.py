...
def fetch_file(filename: str = ""):
    ...
    if ext not in (".png", ".jpg", ".jpeg", ".webp"):
        raise ValueError(
            f"File cannot be fetched: {filename}. Only png and jpg and webp."
        )
...
def create_ui(container, button, tabname):
    ...
    with gr.Tabs(elem_id=tabname + "_extra_tabs"):
        for page in ui.stored_extra_pages:
            ...
            with gr.Tab(page.title, id=page_id):
                ...
                page_elem.change(
                    fn=lambda: None,
                    _js="function(){applyExtraNetworkFilter("
                    + json.dumps(tabname)
                    + "); return []}",
                    inputs=[],
                    outputs=[],
    ...
    button.click(
        ...
        inputs=[state_visible],
        outputs=[state_visible, container, button],
        show_progress=False,
    )
