...
def install_extension_from_index(url, hide_tags, sort_column, filter_text):
    ...
    code, _ = refresh_available_extensions_from_data(
        hide_tags, sort_column, filter_text
    )
...
def refresh_available_extensions_from_data(hide_tags, sort_column, filter_text=""):
    ...
    installed_extension_urls = {
        normalize_git_url(extension.remote): extension.name
            ...
            with gr.TabItem("Installed", id="installed"):
                with gr.Row(elem_id="extensions_installed_top"):
                    ...
                    extensions_disabled_list = gr.Text(
                        elem_id="extensions_disabled_list", visible=False
                    ).style(container=False)
                    extensions_update_list = gr.Text(
                        elem_id="extensions_update_list", visible=False
                    ).style(container=False)
...
<span style="color: var(--primary-400);">
    "Disable all extensions" was set, change it to "none" to load all extensions again
            ...
            with gr.TabItem("Available", id="available"):
                ...
                with gr.Row():
                    search_extensions_text = gr.Text(label="Search").style(
                        container=False
                    )
